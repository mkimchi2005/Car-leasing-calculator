...
def income_tax(gross: float, credit_points: float,
    ...
    if area_pct > 0:
        tax -= min(gross * area_pct / 100.0, area_annual_cap / 12.0)
    return max(0.0, tax)
...
def calculate_all(sal: SalaryInput, cars: list) -> dict:
    ...
    cur_it        = income_tax(cur_taxable, cp, ap, ac)
    ...
    nc_it         = income_tax(nc_taxable, cp, ap, ac)
    ...
    for car in cars:
        ...
        it        = income_tax(taxable, cp, ap, ac)
        ni        = national_insurance(taxable)
        hi        = health_insurance(taxable)
        model_net = cash - it - ni - hi - sal.social_deductions - deficit
